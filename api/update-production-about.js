// Script to update the production About document with new fields
// This will update the About document on Render.com

import fetch from 'node-fetch';

const updateProductionAbout = async () => {
  try {
    console.log('Updating production About document...');
    
    // First, get the current About data
    ...
    const updateData = {
      ...
      vision: currentData.vision || "To become India's most trusted digital real estate network driven by AI and community insights, where every property transaction is transparent, efficient, and customer-centric.",
      coreValues: currentData.coreValues && currentData.coreValues.length > 0 ? currentData.coreValues : [
        {
          title: "Transparency",
      ...
      howItWorks: currentData.howItWorks || {
        buyers: {
      ...
      teamMembers: currentData.teamMembers && currentData.teamMembers.length > 0 ? currentData.teamMembers : [
        {
          name: "Rajesh Kumar",
          role: "CEO & Founder",
      ...
      faqs: currentData.faqs && currentData.faqs.length > 0 ? currentData.faqs : [
        {
    ...
    // Update the About document via the API
    const updateResponse = await fetch('https://urbansetu.onrender.com/api/about', {
      method: 'PUT',
      headers: {
    ...
    if (!updateResponse.ok) {
      throw new Error(`Failed to update About data: ${updateResponse.status}`);
    }
    
    const updatedData = await updateResponse.json();
    console.log('Successfully updated About document!');
    
    // Verify the update
    console.log('\nVerification:');
    console.log('- Has coreValues:', !!updatedData.coreValues, `(${updatedData.coreValues?.length || 0} items)`);
    console.log('- Has howItWorks:', !!updatedData.howItWorks);
    console.log('- Has journey:', !!updatedData.journey, `(${updatedData.journey?.milestones?.length || 0} milestones)`);
    console.log('- Has faqs:', !!updatedData.faqs, `(${updatedData.faqs?.length || 0} items)`);
    console.log('- Has teamMembers:', !!updatedData.teamMembers, `(${updatedData.teamMembers?.length || 0} members)`);
    console.log('- Has socialLinks:', !!updatedData.socialLinks);
    
  } catch (error) {
    console.error('Error updating production About document:', error);
  }
};

updateProductionAbout();
